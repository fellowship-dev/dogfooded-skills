...
`specs/<number>-<slug>/` is the authoritative store for active implementation specs. `docs/plans/` contains non-normative delivery records and `docs/evidence/` contains immutable verification receipts; executable skill behavior remains owned by `skills/`. `.specify/` holds vendored Spec-Kit scaffolding (scripts, templates) that `setup-speckit` populates into target repos and that `speckit-runner` prompts a worker to bootstrap from at runtime — it is tooling, not a spec store.
