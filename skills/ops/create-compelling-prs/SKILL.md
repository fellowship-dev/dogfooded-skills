...
### Pre-PR: Visual evidence (UI changes only)

Before drafting the PR body, detect whether this PR touches UI files:

```bash
UI_CHANGED=false
CHANGED_FILES=$(git diff --name-only $(git merge-base HEAD origin/main) HEAD)

if [ -f .pylot/ui-paths ]; then
  ...
  if echo "$CHANGED_FILES" | grep -qE '\.(tsx?|jsx?|vue|svelte|css|scss|sass|less|html|erb)$'; then
    UI_CHANGED=true
  fi
fi
```

If `UI_CHANGED=true`: invoke `/visual-evidence` and capture the S3 URL(s) returned. Embed them in the `## Visual Evidence` section of the PR body.

If `UI_CHANGED=false`: skip `/visual-evidence`. Omit or delete the `## Visual Evidence` section from the PR body.
